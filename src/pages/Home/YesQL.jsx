...
const YesQL=({ yesQLRef }) => {
  ...
  const handleYesQL = (val) => {
    if (val === 1) {
      ...
      trackEvent({ category: 'Home-YesQl', action: 'click-event', href: 'https://documentia.ai/', name: 'KnowMore-Button', });

    } else if (val === 2) {
      router.push('/contact-us')
      trackEvent({ category: 'Home-YesQl', action: 'click-event', name: 'ContactUs-Button', href: window.location.origin + '/contact-us', })
    };
  }

  const handleKnowMore = () => {
    const url = window.location.origin + '/contact-us';
    trackEvent({ category: 'Home-YesQl', action: 'click-event', name: 'knowMore-Button', href: url, });
    router.push('/contact-us')
  };
  return (
    <div ref={yesQLRef} >
        ...
        {(isInView) => (
          <motion.div className=' py-5 flex flex-col items-center gap-10 w-full' >
            ...
            >
              ...
              <h2 className='font-semibold md:text-6xl text-3xl text-primary max-w-[1024px]'>yesQL™</h2>
              <p className='text-tertiary md:text-xl text-lg max-w-[768px]'>yesQL™ is your go-to tool for effortlessly extracting actionable insights from databases, transforming complex data into clear, impactful results.</p>
            </motion.div>

            <div className='grid md:grid-cols-3 grid-cols-1 gap-5'>
              {yesQLSection.map((data, idx) => (
            ...
            <div className='flex gap-2 justify-start md:justify-center flex-wrap md:flex-row flex-col w-full '>
              <Button name={"Know more"} className={"bg-bg-secondary justify-center text-secondary md:w-auto w-full"} rightIcon={<TbArrowDownRight />} onClick={() => handleYesQL(1)} />
              <Button name={"Contact us"} className={"bg-[#1B55F5] justify-center text-white"} onClick={() => handleYesQL(2)} />
            </div>
          </motion.div>
        )}
      </IsInView>
